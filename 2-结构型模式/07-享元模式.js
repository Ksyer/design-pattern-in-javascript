class Circle {
  ...
  draw() {
    console.log(
      'Circle: Draw() [Color : ' + this.color + ', x : ' + this.x + ', y :' + this.y + ', radius :' + this.radius
    )
  }
}
...
class ShapeFactory {
  static getCircle(color) {
    let circle = ShapeFactory.circleMap.get(color) // {2}
    if (circle == null) {
      circle = new Circle(color)
      ShapeFactory.circleMap.set(color, circle) // {3}
      console.log('Creating circle of color : ' + color)
    }
    return circle
  }
}
ShapeFactory.circleMap = new Map() // {1}
...
/**
 * title: 享元模式
 * scene: 有一个圆，我们想要不同颜色的圆来使用
 * conclusion: 利用Map或其它方式减少重复创建相同类型的实例对象
 */
